...
export function useOutsideClick(ref: React.RefObject<HTMLElement>, handler: (event: MouseEvent | TouchEvent) => void) {
  useEffect(() => {
...
export function ExpandableCardDemo() {
  ...
  // Format certificate data to match the cards format
  const cards = certificationsData.map(cert => ({
    title: cert.title,
    description: cert.issuer,
    src: cert.badgeUrl,
    ctaText: "Verify",
    ctaLink: cert.credentialUrl,
    content: () => {
      const issueDate = new Date(cert.issueDate).toLocaleDateString('en-US', { month: 'long', year: 'numeric' });
      const expiryDate = cert.expiryDate ? new Date(cert.expiryDate).toLocaleDateString('en-US', { month: 'long', year: 'numeric' }) : 'No Expiration';
      ...
      return (
        <div>
          ...
          <p>
            This certification validates expertise in {cert.skills.join(", ")}.
            Click the verify button to view the credential details on the issuer's platform.
          </p>
        </div>
      );
    },
            ...
            >
              <motion.div layoutId={`image-${active.title}-${id}`}>
                <img
                  ...
                  src={active.src}
                  alt={active.title}
                  className="w-full h-80 lg:h-80 sm:rounded-tr-lg sm:rounded-tl-lg object-contain object-center p-4 bg-gray-100 dark:bg-gray-800"
                />
              </motion.div>
              ...
              <div>
                <div className="flex justify-between items-start p-4">
                  ...
                  <motion.a
                    ...
                    className="px-4 py-3 text-sm rounded-full font-bold bg-blue-600 hover:bg-blue-700 text-white"
                  >
                    {active.ctaText}
                  </motion.a>
                </div>
      ...
      <ul className="max-w-2xl mx-auto w-full grid grid-cols-1 md:grid-cols-2 items-start gap-4">
        {cards.map((card) => (
          <motion.div
            ...
            className="p-4 flex flex-col hover:bg-gray-50 dark:hover:bg-gray-800 rounded-xl cursor-pointer"
          >
            <div className="flex gap-4 flex-col w-full">
              <motion.div layoutId={`image-${card.title}-${id}`}>
                <img
                  ...
                  src={card.src}
                  alt={card.title}
                  className="h-60 w-full rounded-lg object-contain object-center p-4 bg-gray-100 dark:bg-gray-800"
                />
              </motion.div>
              <div className="flex justify-center items-center flex-col">
                <motion.h3
                  layoutId={`title-${card.title}-${id}`}
                  className="font-medium text-gray-800 dark:text-gray-200 text-center md:text-left text-base"
                >
                  {card.title}
                </motion.h3>
                <motion.p
                  layoutId={`description-${card.description}-${id}`}
                  className="text-gray-600 dark:text-gray-400 text-center md:text-left text-base"
                >
                  {card.description}
                </motion.p>
              </div>
